...
void getAllCycleDFS(int cur_v){
    ...
    for(int adj=0;adj<10;adj++){
        if(adjmat[cur_v][adj]==0) continue;
...
int main(){
    ...
    for(auto it=fbl.cbegin(); it!=fbl.cend(); it++){
        bit_val+=pow(2, *it-1);
        if(max_latency< *it) max_latency= *it;
    }
    ...
    while(!st_queue.empty()){ //loop until state queue is empty
        ...
        for(int i=1; i< max_latency+1; i++){ 
            ...
            if(cur_bit==false){ //if the bit is 0,which means this new state is permissionable
                auto new_st = sft_st | clv;
                auto new_it = find(vt.begin(), vt.end(), new_st); 
                if(new_it!=vt.end()){ //if the new state has been put in the vertex set
                ...
                else{ //if the new state has not been put into the vertex set 
